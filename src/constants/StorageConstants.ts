// Secure Storage
export const BARFRIENDS = 'BARFRIENDS'
export const DEVICE_MANAGER = 'devicemanager'
export const AUTHORIZATION = 'authorization'
export const DEVICE_TOKEN = 'device_token'
export const REFRESH_TOKEN = 'refresh_token'
export const TERMS_SERVICES = 'terms_services'
// Local Storage
export const LOCAL_STORAGE_SEARCH_AREA = '@local_search_area'
export const LOCAL_STORAGE_PREFERENCE_THEME_COLOR_SCHEME = '@color_theme'
export const LOCAL_STORAGE_PREFERENCE_NOTIFICATIONS_PERMISSION = '@preference_notifications'
export const LOCAL_STORAGE_PREFERENCE_SYSTEM_OF_UNITS = '@preference_system_of_units'
export const LOCAL_STORAGE_PREFERENCE_ASK_BACKGROUND_LOCATION = '@ask_background_location'
export const LOCAL_STORAGE_PREFERENCE_ASK_FOREGROUND_LOCATION = '@ask_foreground_location'
